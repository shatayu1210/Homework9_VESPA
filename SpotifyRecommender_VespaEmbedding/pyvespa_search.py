...
def query_songs_by_embedding(embedding_vector):
    query = {
        'hits': 5,
...
query = "Avicii Silhouettes" # Query for Keyword, Semantic Search

df = keyword_search(app, query)
print(f"Keyword Search Results with Query: {query}\n{df.head()}\n") # Printing Keyword Search Results

df = semantic_search(app, query)
print(f"Semantic Search Results with Query: {query}\n{df.head()}\n") # Printing Semantic Search Results

print("Recommendation Results through Embedding:\n")
emb = get_embedding("17xAiso6mNWpcNQ82JOsAa")
print("Using below embedding for recommendations:",emb)
results = query_songs_by_embedding(emb["fields"]["embedding"])
df = display_hits_as_df(results, ["doc_id", "title", "text"])
print(f"\nEmbedding Recommendations: \n{df.head()}") # Printing Embedding Search Results